...
function countVowels(str) {
  ...
  for (let i = 0; i < strLower.length; i += 1) {
    if (vowels.indexOf(strLower[i]) > -1) count += 1;
  }
...
function isPalindrome(str) {
  const strLowerLetter = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
  const strRevers = strLowerLetter.split('').reverse().join('');

  return strLowerLetter === strRevers;
}
...
function findLongestWord(sentence) {
  ...
  const indexMax = wordsLength.indexOf(Math.max(...wordsLength));
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    arrRevers.push(arr[i].split('').reverse().join(''));
  }
...
function invertCase(str) {
  const arr = str.split('');
  const arrUpper = str.toUpperCase().split('');
  const arrRes = [];

  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] === arrUpper[i]) {
      arrRes.push(arr[i].toLowerCase());
    } else {
      arrRes.push(arrUpper[i]);
    }
  }
...
function getCardId(value) {
  ...
  return 13 * rowArr.indexOf(valueArr[1]) + colArr.indexOf(valueArr[0]);
}
